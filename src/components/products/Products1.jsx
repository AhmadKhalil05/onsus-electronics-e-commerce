...
export default function Products1() {
  ...
  useEffect(() => {
    dispatch({ type: "FULL_RESET", payload: catalog });
  }, [catalogKey]);
  const {
    price,
    isNew,
  ...
  useEffect(() => {
    ...
    if (isNew !== "All") {
      const filteredByisNew = [...catalog].filter((elm) => {
        if (isNew) {
          return elm.inNew;
        } else {
          return !elm.inNew;
    ...
    if (deals !== "All") {
      if (deals == "All Discounts") {
        const filteredBydeails = [...catalog].filter((elm) => elm.oldPrice);
        filteredArrays = [...filteredArrays, filteredBydeails];
      }
      if (deals == "Today’s Deals") {
        const filteredBydeails = [...catalog].filter(
    ...
    if (rating !== "All") {
      const filteredByrating = [...catalog].filter(
        (elm) => elm.rating >= rating
      );
      filteredArrays = [...filteredArrays, filteredByrating];
    }

    const filteredByPrice = [...catalog].filter(
              ...
              <div className="meta-filter-shop" style={{}}>
                ...
                <div id="applied-filters">
                  ...
                  {brands.map((elm, i) => (
                    ...
                    >
                      {brandOptions.find((b) => b.id === elm)?.label ?? elm}
                  ...
                  {(price[0] != 0 || price[1] != 1200) && (
                    <span
                      className="filter-tag"
                      onClick={() => allProps.setPrice([0, 1200])}
                    >
                      ${price[0]} to ${price[1]}
                      <span
