...
export default function Header4({ fullWidth = false }) {
  const { cartProducts, wishList } = useContextElement();
  const cartCount = cartProducts.reduce((n, p) => n + (p.quantity || 1), 0);
            ...
            <div className="col-xxl-9 col-8">
              <div className="header-bt-left active-container">
                <div className="nav-category-wrap">
                  <Link
                    to="/products"
                    className="nav-title btn-active link text-decoration-none"
                  >
                    <i className="icon-menu-dots fs-20" />
                    <h6 className="title fw-semibold mb-0">All products</h6>
                  </Link>
                </div>
